...
- Role: Durable risk register

## SOURCE OF TRUTH

- This file is canonical for persistent risk classes and mitigations.
- Active time-bounded blockers are tracked in `03_OPERATIONS/CURRENT_BLOCKERS.md`.

## DO NOT DUPLICATE

- Do not mirror full blocker dashboards here.
- Link blocker IDs/dates instead of re-copying operational status logs.
...
- Map ABW runtime fields to NVIDIA UI states explicitly in `02_ARCHITECTURE/BRIDGE_CONTRACT.md`.
...
- Record roadmap changes in `05_DECISIONS/DECISIONS.md`.
- Update `05_DECISIONS/ROADMAP.md`, `03_OPERATIONS/HANDOFF.md`, and `07_HISTORY/INTEGRATION_JOURNAL.md` together.
...
## 2026-05-04 Update - Workspace Switch Path Validation Fix Completion Follow-up Risks

- Current risk:
  - code-level workspace switch fix may be mistaken as manual validation closure
...
- Mitigation:
  ...
  - keep `nvidia_playground.html` and `docs/fix-proposal.md` out-of-scope unless separately reviewed
  - keep `DAILY_USE_READY` and production/security/bridge/packaging claims forbidden
  - preserve workspace switch smoke and regression tests

## 2026-05-04 Update - Targeted Manual Path Revalidation Gate Risks
...
- Mitigation:
  - require targeted manual path revalidation rerun
  - keep out-of-scope dirty files excluded
  - classify result as:
    - `MANUAL_PATH_REVALIDATION_PASS`
...
## 2026-05-10 Update - Reaffirmed Manual Path Revalidation V2 Risks
...
## 2026-05-10 Update - Targeted Manual Path Revalidation Rerun V3 Risks

- Current risk:
  - code/regression pass may be mistaken as manual validation closure
  - real UI path workflow may still expose user-facing issues
  - provider/rate guard may still interrupt user flow, though now separately classified
  - out-of-scope NVIDIA dirty files may be accidentally committed later
  - `nvidia_playground.html` remains dirty due intentional Vietnamese localization
  - `docs/fix-proposal.md` remains untracked
  - moving to runtime/security or another Builder before manual rerun could leave practical workflow unverified
- Mitigation:
  - require targeted manual path revalidation rerun V3
  - keep out-of-scope dirty files excluded
  - classify result as:
...
## 2026-05-13 Update - Post Manual Path Revalidation V3 Pass Residual Risks

- Closed for deterministic NVIDIA validation path:
  - previous real UI provider/tool-calling blocker recorded under `MANUAL_PATH_REVALIDATION_V3_FAIL`
  - provider unsupported path is now classified as `PROVIDER_TOOL_CALLING_UNSUPPORTED`
...
- Residual risks remain:
  ...
  - enterprise-grade security is still not proven
  - packaging readiness is still not proven
...
- Closed:
  - bounded pilot procedure now exists in control governance as `06_VALIDATION/PILOT_CHECKLIST.md`
...
- Closed in this planning-only step:
  - data policy/sanitization/safety/pass-fail/stop-condition/rollback planning artifact is now explicit in `06_VALIDATION/SMALL_REAL_WORK_DOC_PILOT_PLAN.md`
