...
- scope:
  - NVIDIA wording fix plus CONTROL audit-trail update only
- preserved history:
  - the real browser pilot remains recorded as `WARNING_BOUNDED_NON_TECH_APPROVE_UI_PILOT_AFTER_RETRIEVAL_FIX`
  - the warning is preserved because the pilot ran before the wording fix
- NVIDIA follow-up:
  - Step 3 / review-panel wording no longer says trusted-source approval is unavailable
  - triage copy now says approval is available only for one selected source after successful preview and explicit confirmation
  - candidate marking is described as review-only, not approval
  - wording still avoids approve-all, batch approval, corpus approval, and readiness overclaim
- validation:
  ...
  - `npm test` PASS
- interpretation:
  - safety/honesty result from the pilot is unchanged
  - this follow-up does not promote readiness
  - next gate can move to `RERUN_COPY_CLEAN_BOUNDED_NON_TECH_APPROVE_UI_PILOT`

## 2026-05-18 - Bounded non-tech approve UI pilot rerun after retrieval fix recorded as warning
...
- warning reason:
  ...
  - review summary/action copy still implies manual-only promotion
...
- NVIDIA bridge/UI evidence:
  ...
  - supplier contract query: `ABW_CLI_NO_MATCH`, displayed `Kh・・ｽｴng t・・ｽｬm th陂ｯ・･y th・・ｽｴng tin ・・ｦ･・｡ng tin c陂ｯ・ｭy.`, trust `0`, `E0_unknown`, no sources, `gapLogSuppressed=true`, `wouldLogGap=true`
...
- prompt:
  `vi陂ｯ・ｿt cho t・・ｽｴi ch・・ｽｰ・・ｽ｡ng tr・・ｽｬnh t・・ｽｭnh t逶ｻ雋ｧg 2 s逶ｻ繝ｻA+B v・・｣ｰ ・・ｦ･・ｳng g・・ｽｳi n・・ｽｳ th・・｣ｰnh m逶ｻ蜀ｲ file`
- inferred target:
  `proof/sum_ab.py`
- pending edit:
  created
...
## 2026-05-02 - NVIDIA Sprint 15 Project Rules / Memory UI t鬯ｨ・ｾ繝ｻ・ｶ郢晢ｽｻ繝ｻ・ｻ鬯ｮ・ｫ繝ｻ・ｪ郢晢ｽｻ繝ｻ・ｴ thi鬯ｨ・ｾ繝ｻ・ｶ郢晢ｽｻ繝ｻ・ｻ鬩幢ｽ｢隴擾ｽｴ郢晢ｽｻcommitted and pushed
...
## 2026-05-02 - NVIDIA Sprint 13 Git / SCM Panel t鬯ｯ・ｨ繝ｻ・ｾ郢晢ｽｻ繝ｻ・ｶ驛｢譎｢・ｽ・ｻ郢晢ｽｻ繝ｻ・ｻ鬯ｮ・ｯ隶抵ｽｭ繝ｻ繝ｻ・ｹ譎｢・ｽ・ｻh鬩幢ｽ｢隴趣ｽ｢繝ｻ・ｽ繝ｻ・ｻ鬩幢ｽ｢隴趣ｽ｢繝ｻ・ｽ繝ｻ・ｻ驛｢譎｢・ｽ・ｻ郢晢ｽｻ繝ｻ・ｽ驛｢譎｢・ｽ・ｻ郢晢ｽｻ繝ｻ・｡n committed and pushed
...
## 2026-05-17 - NVIDIA preview-only approve dry-run Stage D evidence recorded
...
- recorded NVIDIA delta:
  ...
  - UI calls `/proxy/abw/approve-draft` with `dry_run=true` only
  - no confirm payload sent from the UI
  - preview panel shows draft source path, current `Not trusted yet` state, target trusted source path if returned, preview summary, warnings, blocking errors, and future confirmation token/text only
  - copy states preview only, nothing was changed, approval comes later, one source does not approve the folder, and blocked preview changes nothing
- preserved boundaries:
...
- interpretation:
  - Stage D closes the missing preview-only dry-run UI implementation gap
  - this does not implement single-item approve apply
